...
const HospitalDepartments: React.FC<HospitalDepartmentsProps> = ({ departments }) => {
  ...
  return (
    <div className="bg-white p-4 text-center rounded-[15px] shadow w-[50vw] max-sm:w-[90vw]">
      <div className='flex flex-row justify-center items-center gap-[1vw] mb-[3vh] max-sm:text-center'>
        <img src='public/hospital-departments.svg' className='h-[5vh] max-sm:h-[3vh]' />
            ...
            return (
              <div key={index} className="flex flex-col justify-around mb-2 text-primary">
                ...
                <div className='flex flex-row justify-center items-center gap-[0.5vw]'>
                  <p>Temps d'attente moyen : {department.averageWaitTime}</p>
                  {isNaN(waitTime) ? (
                    <p>Temps d'attente non disponible</p>
                  ) : waitTime < 30 ? (
                    <img src="/chronometer-green.svg" alt="Chronomètre vert" className='h-[3vh]' />
                  ) : waitTime < 45 ? (
                    <img src="/chronometer-orange.svg" alt="Chronomètre orange" className='h-[3vh]' />
                  ) : (
                    <img src="/chronometer-red.svg" alt="Chronomètre rouge" className='h-[3vh]' />
                  )}
                </div>
              </div>
            );
